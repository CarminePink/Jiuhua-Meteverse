...
import { buildLoginRedirectQuery } from '@/router/query'
/**
 * 创建并配置一个 Axios 实例对象
 */
...
const handleResponse = (response: AxiosResponse<any>) => {
    ...
    if (response.data.encryption) {
        const originData = JSON.parse(
            other.decryption(response.data.encryption, import.meta.env.VITE_PWD_ENC_KEY)
        )
...
service.interceptors.response.use(handleResponse, error => {
    ...
    if (status === 424) {
        ElMessageBox.confirm('令牌状态已过期，请点击重新登录', '提示', {
            ...
            router.push({
                path: '/login',
                query: buildLoginRedirectQuery(currentRoute)
            })
            return
        })
    }
